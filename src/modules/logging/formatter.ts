...
export class LogFormatter {
	public static format(logRecord: LogRecord): string {
		...
		for (const line of logRecord.msg.split("\n")) {
			msg += `${LogFormatter.formatDate(logRecord.datetime)} | ${logRecord.loggerName.substr(0, 8).padEnd(8)} | ${logRecord.levelName.padEnd(8)} | ${line}\n`
		}
		return msg.trim();
	}
	...
	public static formatDate (date: Date = new Date(), options?: { file?: boolean }): string {
		...
		const timeString: string = file ? `${date.getHours().toString().padStart(2, "0")}-${date.getMinutes().toString().padStart(2, "0")}-${date.getSeconds().toString().padStart(2, "0")}`
